...
StilFant1DakisSynthDefs {
	*load {
		...
		*/
		...
		SynthDef(\play1, {| out = 0, bufnum = 0, amp = 8|
			var rate;
			rate = 1/4;
			Out.ar(out, 
				PlayBuf.ar(2, ~buffer1.bufnum, BufRateScale.kr(bufnum)*rate, loop: 1.0) * amp
			)
		}).send(s);
		
		SynthDef(\play2, {| out = 0, bufnum = 1, amp = 1|
			var rate;
			rate = 1/6;
			Out.ar(out, 
				PlayBuf.ar(2, ~buffer2.bufnum, BufRateScale.kr(bufnum)*rate, loop: 1.0)  * amp
			)
		}).send(s);
		
		SynthDef(\play3, {| out = 0, bufnum = 2, amp = 1|
			var rate;
			rate = 1;
			Out.ar(out, 
				PlayBuf.ar(2, ~buffer3.bufnum, BufRateScale.kr(bufnum)*rate, loop: 1.0)  * amp
			)
		}).send(s);
		
		SynthDef(\play4, {| out = 0, bufnum = 3, amp = 1 |
			var rate;
			rate = 1;
			Out.ar(out, 
				PlayBuf.ar(2, ~buffer4.bufnum, BufRateScale.kr(bufnum)*rate , loop: 1.0)  * amp
			)
		}).send(s);
	}
}